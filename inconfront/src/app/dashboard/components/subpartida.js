...
class SubpartidaRepository {
  ...
  async createSubpartida(subpartida) {
    try {
      ...
      // Primero, creamos la subpartida en la base de datos
      ...
      try {
        ...
        console.error('Error al crear la estructura de carpetas para la subpartida:', folderError);
        // No lanzamos el error para permitir que la subpartida se cree aunque falle la estructura de carpetas
      }

      return subpartidaCreated;
    } catch (error) {
      console.error('Error al crear la subpartida:', error);
      throw error;
    }
  }
